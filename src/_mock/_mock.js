...
const _mock = {
  ...
  image: {
    cover: (index) => {
      const coverIndex = ((index) % 25) + 1;
      return `/assets/images/cover/cover-${coverIndex}.webp`;
    },
    product: (index) => {
      const productIndex = ((index) % 25) + 1;
      return `/assets/images/m-product/product-${productIndex}.webp`;
    },
    avatar: (index) => {
      const avatarIndex = ((index) % 25) + 1;
